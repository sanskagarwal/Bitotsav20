...
function fetchProfile() {
    ...
    let user = null;
    $.ajax({
        url: url1 + "/api/dashboard/getProfile",
        method: "GET",
        headers: {
...
function displayMembers() {
    ...
    if (userDetails === null) {
        ...
        if (isInTeam) {
            html += `
                <p>You team ${teamDetails.teamName} will be registered for this event.</p>
...
function displayTeamRegistrationButton() {

    let html = ``;
    if (userDetails && isInTeam) {
        html += `<button id="events${i}TeamRegisterButton" class="btn btn-success">Register</button>`
    }
    return html;
}


function teamRegFormSubmit() {
    const minParticipants = event.minParticipants;
    const maxParticipants = event.maxParticipants;
    ...
    if (indices.length < minParticipants && indices.length > maxParticipants) {
        ...
        const members = [];
        indices.forEach((i) => {
            if (teamDetails[i].email === userDetails.email) leaderFound = true;
            members.push({
                email: teamDetails.email,
                bitotsavId: teamDetails.bitotsavId
            });
        });
...
function eventdetails(event, i, s) {
    ...
    <div class="col-md-4">
        <div class="card" style="margin-bottom: 20px">
            <img class="card-img-top img-fluid" height="300" src="./images/Events/allEvents/${events.imageName}"
                alt="${event.name}">
            <div class="card-body">
                <hr>
                ${event.name}
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        ${displayTeamRegistrationButton(i)}
                        <button type="button" class="btn btn-success">Register</button>
                        <p id="events${i}TeamRegisterErrMsg"></p>
                    </div>
                </div>
            </div>
        </div>
